...
from core.dataset_dy import TrainDataset
...
class Trainer:
    def __init__(self, config):
        ...
        self.netG = net.InpaintGenerator(in_channel=5, out_channel=1)
    ...
    def _train_epoch(self, pbar):
        ...
        for frames, depths, con_mask, occ_mask, fore_flow, back_flow, name in self.train_loader:
            ...
            masked_depths = torch.cat([frames * con_mask, depths * con_mask, 1 - con_mask], dim=2)
            ...
            pred_imgs = self.netG(masked_depths, pred_flows_bi, 1 - con_mask)
            pred_imgs = torch.tanh(pred_imgs)
            pred_imgs = pred_imgs.view(b, -1, 1, h, w)
            comp_imgs = depths * con_mask + pred_imgs * occ_mask

            if self.config['global_rank'] == 0 and self.iteration % 500 == 0:
                aa = (comp_imgs[0, 2, :1, :, :].permute(1, 2, 0).detach().cpu().numpy()) *255
                cv2.imwrite("1.png", aa)
                dd = ((pred_imgs * occ_mask)[0, 2, :1, :, :].permute(1, 2, 0).detach().cpu().numpy() ) *255
                cv2.imwrite("2.png", (dd))
                ee = ((depths * occ_mask)[0, 2, :1, :, :].permute(1, 2, 0).detach().cpu().numpy()) * 255
            ...
            hole_loss = self.l1_loss(pred_imgs * occ_mask, depths * occ_mask)
            ...
            valid_loss = self.l1_loss(pred_imgs * con_mask, depths * con_mask)
            ...
            if not self.config['model']['no_dis']:
                ...
                real_clip = self.netD(depths * (con_mask+occ_mask))
