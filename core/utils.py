...
class GroupRandomHorizontalFlowFlip(object):
    ...
    """
    def __call__(self, img_group, dep_group, flowF_group, flowB_group):
        v = random.random()
        if v < 0.5:
            ret_img = [
            ...
            ret_dep = [
                dep.transpose(Image.FLIP_LEFT_RIGHT) for dep in dep_group
            ]
            ret_flowF = [ff[:, ::-1] * [-1.0, 1.0] for ff in flowF_group]
            ret_flowB = [fb[:, ::-1] * [-1.0, 1.0] for fb in flowB_group]
            return ret_img, ret_dep, ret_flowF, ret_flowB
        else:
            return img_group, dep_group, flowF_group, flowB_group
