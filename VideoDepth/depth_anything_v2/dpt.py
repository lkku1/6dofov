...
class DepthAnythingV2(nn.Module):
    ...
    # @torch.no_grad()
    # def infer_image(self, raw_image, max_depth, input_size=518):
    ...
    @torch.no_grad()
    def infer_image(self, image, size, max_depth):
        # image, (h, w) = self.image2tensor(raw_image, input_size)

        depth = self.forward(image, max_depth)

        depth = F.interpolate(depth[:, None], size, mode="bilinear", align_corners=True)
