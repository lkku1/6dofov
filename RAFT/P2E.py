...
# def pers2equi(pers_img, fov, nrows, patch_size, erp_size, layer_name, index=100):
#     bs = pers_img.shape[0]
...
#         # if index != 100:
#         #     temp = all_combos[index]
#         #     all_combos = []
#         #     all_combos.append(temp)
#         #     all_combos = np.vstack(all_combos)
#
...
#     w_list = F.normalize(w_list, p=1, dim=-1).reshape(erp_h, erp_w, -1, 4)
#     w_list = w_list.unsqueeze(0).unsqueeze(0)
#     output = output_a[..., index] * w_list[..., index,0] + output_b[..., index] * w_list[...,index,1] + \
#              output_c[..., index] * w_list[..., index,2] + output_d[..., index] * w_list[..., index,3]
#     # mask = torch.where(output > 0, 1, 0)
#     # mask = mask.sum(-1)
#
#     return output
...
def pers2equi(pers_img, fov, nrows, patch_size, erp_size, layer_name):
    ...
    w_list = w_list.unsqueeze(0).unsqueeze(0)
    # bs, 1, h, w, n_patch
    output = output_a * w_list[..., 0] + output_b * w_list[..., 1] + output_c * w_list[..., 2] + output_d * w_list[..., 3]

    color = output[:, :2]
    alpha = output[:, 2:]
    alpha_sum = torch.sum(alpha, dim=-1, keepdim=True)

    output = color * (alpha / alpha_sum)
    output[output.isnan()] = 0
    output = output.sum(-1).type(torch.float32)
        ...
        if (position_list[i][col] == position_list[point_index][col]):
            ...
            for j in range(num):
                ...
                if num == shift:
                    ...
                    weight[point_index + j] = (position_list[point_index + j][index] - position_list[point_index][index]) / num
