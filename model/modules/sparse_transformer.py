...
from model.modules.conv_pad import PadConv2d

class SoftSplit(nn.Module):
    def __init__(self, channel, hidden, kernel_size, stride):
        super(SoftSplit, self).__init__()
        self.kernel_size = kernel_size
        self.stride = stride
        self.t2t = nn.Unfold(kernel_size=kernel_size, stride=stride)
        c_in = reduce((lambda x, y: x * y), kernel_size) * channel
        self.embedding = nn.Linear(c_in, hidden)

    def forward(self, x, b, x_size, mode):
        # x_size = torch.tensor(x_size)

        pad_y_top = int((self.kernel_size[0] - 1) / 2)
        pad_y_bottom = int(torch.ceil(x_size[0] / self.stride[0]) * self.stride[0]) - x_size[0] + int((self.kernel_size[0] - 1) / 2)
        pad_x_left = int((self.kernel_size[1] - 1) / 2)
        pad_x_right = int(torch.ceil(x_size[1] / self.stride[1]) * self.stride[1]) - x_size[1] + int((self.kernel_size[1] - 1) / 2)
        x = F.pad(F.pad(x, pad=(pad_x_left, pad_x_right, 0, 0), mode=mode), pad=(0, 0, pad_y_top, pad_y_bottom))

        bt, _, h, w = x.shape
        f_h = int((h - (self.kernel_size[0] - 1) - 1) / self.stride[0] + 1)
        f_w = int((w - (self.kernel_size[1] - 1) - 1) / self.stride[1] + 1)
        ...
        return feat, (h, w), (pad_y_top, pad_y_bottom, pad_x_left, pad_x_right)
...
class SoftComp(nn.Module):
    def __init__(self, channel, hidden, kernel_size, stride):
        ...
        self.bias_conv = PadConv2d(channel,
                                   channel,
                                   kernel_size=3,
                                   stride=1
                                   )
    ...
    def forward(self, x, t, output_size):
        ...
        b, _, c = feat.size()
        feat = feat.view(b * t, -1, c).permute(0, 2, 1).contiguous()
...
class FusionFeedForward(nn.Module):
    ...
    def forward(self, x, output_size):
        n_vecs = 1
        output_size_pad = torch.zeros_like(output_size)
        for i, d in enumerate(self.t2t_params['kernel_size']):
            pad = int(math.ceil(output_size[i] / self.t2t_params['stride'][i]) * self.t2t_params['stride'][i]) - output_size[i] + int((d - 1))
            n_vecs *= int((output_size[i] + pad - d) / self.t2t_params['stride'][i] + 1)
            output_size_pad[i] = output_size[i] + pad

        x = self.fc1(x)
        b, n, c = x.size()
        normalizer = x.new_ones(b, n, self.kernel_shape).view(-1, n_vecs, self.kernel_shape).permute(0, 2, 1).contiguous()

        normalizer = F.fold(normalizer,
                            output_size=output_size_pad,
                            kernel_size=self.t2t_params['kernel_size'],
                            stride=self.t2t_params['stride'])

        x = F.fold(x.view(-1, n_vecs, c).permute(0, 2, 1).contiguous(),
                   output_size=output_size_pad,
                   kernel_size=self.t2t_params['kernel_size'],
                   stride=self.t2t_params['stride'])
...
class SparseWindowAttention(nn.Module):
    ...
    def forward(self, index, x, y, mask=None, T_ind=None, attn_mask=None):
        ...
        if pad_r > 0 or pad_b > 0:
            ...
            y = F.pad(y, (0, 0, 0, pad_r, 0, pad_b, 0, 0), mode='constant', value=0)
        ...
        # pool_k and pool_v
        if self.pooling_token:
            pool_x = self.pool_layer(x.view(b*t, new_h, new_w, c).permute(0,3,1,2).contiguous())
            _, _, p_h, p_w = pool_x.shape
            pool_x = pool_x.permute(0,2,3,1).contiguous().view(b, t, p_h, p_w, c)
            # pool_k
            pool_k = self.key(pool_x).unsqueeze(1).repeat(1, n_wh*n_ww, 1, 1, 1, 1) # [b, n_wh*n_ww, t, p_h, p_w, c]
            pool_k = pool_k.view(b, n_wh*n_ww, t, p_h, p_w, self.n_head, c_head).permute(0,1,5,2,3,4,6).contiguous()
            ...
            pool_v = pool_v.view(b, n_wh*n_ww, t, p_h, p_w, self.n_head, c_head).permute(0,1,5,2,3,4,6).contiguous()
        ...
        for i in range(win_q.shape[0]):
            ...
            if mask_n > 0:
                ...
                att_t = (win_q_t @ win_k_t.transpose(-2, -1).contiguous()) * (1.0 / math.sqrt(win_q_t.size(-1)))
            ...
            att_s = (win_q_s @ win_k_s.transpose(-2, -1).contiguous()) * (1.0 / math.sqrt(win_q_s.size(-1)))
...
class TemporalSparseTransformer(nn.Module):
    ...
    def forward(self, index, x, y, fold_x_size, mask=None, T_ind=None):
        """
        Args:
        ...
        """
        B, T, H, W, C = x.shape # 43 43

        shortcut = x
        x = self.norm1(x)
        y = self.norm1(y)
        att_x = self.attention(index, x, y, mask, T_ind)

        # FFN
        shortcut = shortcut + att_x
        x = self.norm2(x)
        x = shortcut + self.mlp(x.view(B, T * H * W, C), fold_x_size).view(B, T, H, W, C)
...
class TemporalSparseTransformerBlock(nn.Module):
    ...
    def forward(self, x, y, fold_x_size, l_mask=None):
        """
        Args:
            x: image tokens, shape [B T H W C]
            fold_x_size: fold feature size, shape [60 108]
        ...
        for i in range(0, self.depths):
            x = self.transformer[i](i, x, y, fold_x_size, l_mask)
